...
PATH_TO_CKPT = "./src/DataLayer/ssd_mobilenet_v1_coco_2018_01_28/frozen_inference_graph.pb"
...
while True:
    ...
    for idx in range(int(num)):
        ...
        if score > 0.4   : 
            ...
            #ys = obj_points[:, 1]
            zs = np.delete(zs, np.where(
                (zs < z - 1) | (zs > z + 1)))  # take only y for close z to prevent including background

            z_final = np.median(zs)
           

            depth = z_final  # add next to rectangle print of height using cv library
            depth = float("{:.2f}".format(depth))
            print("[INFO] object depth is: ", depth, "[m]")
            depth_txt = str(depth) + "[m]"
